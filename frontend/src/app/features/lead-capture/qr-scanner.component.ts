    ...
    <div class="card-panel h-full flex flex-col justify-between hover:shadow-md transition bg-white border border-slate-200 rounded-xl p-4">
      <div>
        ...
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center gap-2">
            ...
            <h3 class="text-sm font-bold text-slate-800">QR Code / vCard</h3>
          </div>
          <span class="text-[10px] font-bold uppercase tracking-wider bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded-full flex items-center gap-1">
            <span class="material-icons text-[12px]">badge</span> Badges & vCards
          </span>
        </div>
        ...
        <div class="grid grid-cols-1 gap-2 mb-2">
          <button 
            ...
            class="btn btn-primary w-full justify-center text-xs py-2.5 rounded-lg font-bold shadow-sm flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white"
          >
            <span class="material-icons text-sm">photo_camera</span>
            Scan via Camera
          </button>

          <label class="cursor-pointer block border border-dashed border-indigo-300 hover:border-indigo-500 rounded-lg p-2.5 text-center bg-indigo-50/50 hover:bg-indigo-100/50 transition">
            <input 
              type="file" 
            ...
            />
            <div class="flex items-center justify-center gap-1.5 text-xs font-semibold text-indigo-700">
              @if (isScanningFile()) {
                <span class="material-icons text-sm animate-spin text-indigo-600">sync</span>
      ...
      @if (lastScannedData() && !showModal() && !showCameraModal()) {
        <div class="mt-3 bg-indigo-50 border border-indigo-200 rounded-lg p-3">
          <div class="flex items-center justify-between mb-1 pb-1 border-b border-indigo-100">
            ...
            <button 
              ...
              class="text-[11px] font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-0.5"
            >
          ...
          <button 
            ...
            class="w-full mt-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs py-1.5 px-3 rounded-md transition shadow-xs flex items-center justify-center gap-1"
          >
      ...
      <div class="mt-2 text-right">
        <button 
          type="button" 
          (click)="simulateScan()" 
          class="text-[10px] text-slate-400 hover:text-indigo-600 underline font-medium"
        >
          ⚡ Try Sample vCard QR
        </button>
      </div>
        ...
        <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 animate-fadeIn relative">
          ...
          <div class="relative bg-slate-900 rounded-xl overflow-hidden min-h-[260px] flex items-center justify-center border border-slate-800">
            ...
            @if (isStartingCamera()) {
              <div class="absolute inset-0 bg-slate-900 flex flex-col items-center justify-center text-white text-xs gap-2">
                <span class="material-icons text-3xl animate-spin text-indigo-400">sync</span>
            ...
            @if (cameraError()) {
              <div class="absolute inset-0 bg-slate-900/90 p-4 flex flex-col items-center justify-center text-center text-white text-xs gap-2">
                ...
                <button 
                  ...
                  class="mt-2 bg-indigo-600 text-white font-bold px-3 py-1.5 rounded-lg text-xs"
                >
                  Retry Camera
                </button>
              </div>
          ...
          <div class="mt-4 flex items-center justify-between text-xs text-slate-500">
            <span class="flex items-center gap-1">
              <span class="material-icons text-indigo-500 text-sm">center_focus_strong</span>
    ...
    @if (showModal()) {
      <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 animate-fadeIn">
          <div class="flex items-center justify-between border-b pb-3 mb-4">
            <div class="flex items-center gap-2">
              <span class="material-icons text-indigo-600">verified</span>
          ...
          <div class="space-y-3 max-h-80 overflow-y-auto pr-1">
            <div>
              ...
              <input type="text" [(ngModel)]="modalData.name" class="w-full text-xs p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Sarah Connor" />
            </div>

            <div>
              <label class="block text-xs font-bold text-slate-700 mb-1">Designation / Role</label>
              <input type="text" [(ngModel)]="modalData.designation" class="w-full text-xs p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Operations Director" />
            </div>

            <div>
              <label class="block text-xs font-bold text-slate-700 mb-1">Company Name</label>
              <input type="text" [(ngModel)]="modalData.company" class="w-full text-xs p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Cyberdyne Systems" />
            </div>

            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-bold text-slate-700 mb-1">Mobile / Phone</label>
                <input type="text" [(ngModel)]="modalData.phone" class="w-full text-xs p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="+91 9876543210" />
              </div>

              <div>
                <label class="block text-xs font-bold text-slate-700 mb-1">Email Address</label>
                <input type="email" [(ngModel)]="modalData.email" class="w-full text-xs p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="sarah@cyberdyne.io" />
              </div>
            </div>

            <div>
              <label class="block text-xs font-bold text-slate-700 mb-1">Website URL</label>
              <input type="text" [(ngModel)]="modalData.website" class="w-full text-xs p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="www.cyberdyne.io" />
            </div>

            <div>
              <label class="block text-xs font-bold text-slate-700 mb-1">Address / Location</label>
              <input type="text" [(ngModel)]="modalData.address" class="w-full text-xs p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="City, State, Country" />
            </div>
          </div>
          ...
          <div class="mt-6 flex items-center justify-end gap-2 border-t pt-4">
            ...
            <button type="button" (click)="saveAndApplyModal()" class="px-5 py-2 text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-md flex items-center gap-1">
...
export class QrScannerComponent implements OnDestroy {
  ...
  private async scanQrImageWithFallback(file: File): Promise<string> {
    // Helper to safely execute scan on a given File
    ...
    if (res1) return res1;
    console.log("Strategy 1 (Raw file) failed, trying Canvas Quiet-Zone Padding...");
    ...
    if (res2) return res2;
    console.log("Strategy 2 (Padded margin) failed, trying Non-rendered Canvas scan...");

    // Strategy 3: Non-rendered padded scan
    const res3 = await tryScan(paddedFile, false);
    if (res3) return res3;
    console.log("Strategy 3 (Non-rendered) failed, trying Binarized High-Contrast Canvas...");
